...
use crate::lsps4::client::{LSPS4ClientConfig, LSPS4ClientHandler};
use crate::lsps4::msgs::LSPS4Message;
use crate::lsps4::service::{LSPS4ServiceConfig, LSPS4ServiceHandler};
...
pub struct LiquidityServiceConfig {
	...
	/// Optional server-side configuration for LSPS4 channels.
	pub lsps4_service_config: Option<LSPS4ServiceConfig>,
...
pub struct LiquidityClientConfig {
	...
	/// Optional client-side configuration for LSPS4 channels.
	pub lsps4_client_config: Option<LSPS4ClientConfig>,
...
pub trait ALiquidityManager {
	...
	type KVStore: KVStore + KVStoreSync + ?Sized;
	...
	fn get_lm(
		&self,
	) -> &LiquidityManager<Self::ES, Self::NS, Self::CM, Self::C, Self::K, Self::TP, Self::T, Self::L>;
}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> ALiquidityManager for LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	...
	L::Target: Logger,
{
	...
	type L = L;
	fn get_lm(&self) -> &LiquidityManager<ES, NS, CM, C, K, TP, T, L> {
		self
	}
}
...
pub trait ALiquidityManagerSync {
	...
	fn get_lm(
		&self,
	) -> &LiquidityManagerSync<Self::ES, Self::NS, Self::CM, Self::C, Self::KS, Self::TP, Self::T, Self::L>;
}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> ALiquidityManagerSync for LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	fn get_lm(&self) -> &LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L> {
		self
	}
}
	...
	L::Target: Logger,
{
	...
	lsps4_client_handler: Option<LSPS4ClientHandler<ES, K>>,
	lsps4_service_handler: Option<LSPS4ServiceHandler<CM, K, L>>,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> LiquidityManager<ES, NS, CM, C, K, DefaultTimeProvider, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	///
	/// `logger` is required when LSPS4 service is configured, otherwise it can be `None`.
	pub async fn new(
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	pub async fn new_with_custom_time_provider(
		...
		let lsps4_client_handler = client_config.as_ref().and_then(|config| {
			config.lsps4_client_config.map(|config| {
				LSPS4ClientHandler::new(
					entropy_source.clone(),
					Arc::clone(&pending_messages),
					Arc::clone(&pending_events),
					config.clone(),
		...
		let lsps4_service_handler = if let Some(service_config) = service_config.as_ref() {
			if let Some(lsps4_service_config) = service_config.lsps4_service_config.as_ref() {
				if let Some(number) =
					<LSPS4ServiceHandler<CM, K, L> as LSPSProtocolMessageHandler>::PROTOCOL_NUMBER
				{
					supported_protocols.push(number);
				}

				Some(LSPS4ServiceHandler::new(
					Arc::clone(&pending_messages),
					Arc::clone(&pending_events),
					channel_manager.clone(),
					lsps4_service_config.clone(),
					kv_store.clone(),
					logger.clone(),
				).await?)
			} else {
	...
	/// Returns a reference to the LSPS4 client-side handler.
	///
	/// The returned hendler allows to initiate the LSPS4 client-side flow. That is, it allows to
	/// retrieve all necessary data to create 'just-in-time' invoices that, when paid, will have
	/// the configured LSP open a 'just-in-time' channel.
	pub fn lsps4_client_handler(&self) -> Option<&LSPS4ClientHandler<ES, K>> {
		self.lsps4_client_handler.as_ref()
	}

	/// Returns a reference to the LSPS4 server-side handler.
	///
	/// The returned handler allows to initiate the LSPS4 service-side flow.
	pub fn lsps4_service_handler(&self) -> Option<&LSPS4ServiceHandler<CM, K, L>> {
		self.lsps4_service_handler.as_ref()
	}
	...
	pub async fn persist(&self) -> Result<(), lightning::io::Error> {
		...
		if let Some(lsps4_service_handler) = self.lsps4_service_handler.as_ref() {
			lsps4_service_handler.persist()?;
		}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> CustomMessageReader for LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> CustomMessageHandler for LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	fn peer_disconnected(&self, counterparty_node_id: bitcoin::secp256k1::PublicKey) {
		...
		if let Some(lsps4_service_handler) = self.lsps4_service_handler.as_ref() {
			lsps4_service_handler.peer_disconnected(&counterparty_node_id);
		}
	...
	fn peer_connected(
		...
		if let Some(lsps4_service_handler) = self.lsps4_service_handler.as_ref() {
			lsps4_service_handler.peer_connected(counterparty_node_id);
		}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> Listen for LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> Confirm for LiquidityManager<ES, NS, CM, C, K, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
> where
	...
	L::Target: Logger,
{
	inner: LiquidityManager<ES, NS, CM, C, KVStoreSyncWrapper<KS>, TP, T, L>,
}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> LiquidityManagerSync<ES, NS, CM, C, KS, DefaultTimeProvider, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	pub fn new(
		...
		let kv_store = KVStoreSyncWrapper(kv_store_sync.clone());
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone
	> LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
	...
	L::Target: Logger,
{
	...
	/// retrieve all necessary data to create 'just-in-time' invoices that, when paid, will have
	/// the configured LSP open a 'just-in-time' channel.
	pub fn lsps4_client_handler(&self) -> Option<&LSPS4ClientHandler<ES, KVStoreSyncWrapper<KS>>> {
		self.inner.lsps4_client_handler()
	}

	/// Returns a reference to the LSPS4 server-side handler.
	///
	/// Wraps [`LiquidityManager::lsps4_service_handler`].
	pub fn lsps4_service_handler(&self) -> Option<&LSPS4ServiceHandler<CM, KVStoreSyncWrapper<KS>, L>> {
		self.inner.lsps4_service_handler()
	}
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> CustomMessageReader for LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> CustomMessageHandler for LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> Listen for LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
...
impl<
		...
		T: Deref + Clone,
		L: Deref + Clone,
	> Confirm for LiquidityManagerSync<ES, NS, CM, C, KS, TP, T, L>
where
	ES::Target: EntropySource,
	NS::Target: NodeSigner,
	CM::Target: AChannelManager,
